...
var JPMNLogger = (() => {
  class JPMNLogger {
    ...
    errorStack(stack) {
      try {
        let ignoredErrors = getSetting(['ignored-errors',],['ReferenceError: EFDRC is not defined', '/_anki/js/reviewer.js:7:112035)', '/_anki/js/reviewer.js:7:113091)']);
...
const JPMNKanjiHover = (() => {
  ...
  class JPMNKanjiHover {
    ...
    async cardQueries(kanjiArr) {
      const cardTypeName = 'Japanese Kanji';
      ...
      for (const character of kanjiArr) {
        ...
        let baseQuery = `(-"Key:${keyEsc}" Kanji:*${character}* "deck:Kanji" -"WordReading:${wordReadingEsc}") `;
    ...
    getMoraeOfAJTWord(ajtWord) {
      ...
      const classesToRemove = ['high', 'low', 'high_drop', 'low_rise'];
        ...
        for (const x of temp.childNodes) {
            if (
                (x.nodeName === "SPAN") &&
                (classesToRemove.some(className => x.classList.contains(className)))
            ) {
