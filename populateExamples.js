...
function extractYomitanDefinitions(html) {
  ...
  const senseGroupRegex =
    /<div\s+[^>]*data-sc-content="sense-group"[^>]*>([\s\S]*?)<\/div>\s*(?=<\/div>|<div\s+data-sc-content="sense-group"|<div\s+data-sc-content="attribution"|$)/gi;
  let groupMatch;
  ...
  while ((groupMatch = senseGroupRegex.exec(html)) !== null) {
    ...
    const posRegex =
      /<span\s+[^>]*data-sc-content="part-of-speech-info"[^>]*>([^<]+)<\/span>/gi;
    ...
    const glossaryMatch = groupContent.match(
      /<ul\s+[^>]*data-sc-content="glossary"[^>]*>([\s\S]*?)<\/ul>/i,
    );
    if (glossaryMatch) {
    ...
    const exampleMatch = groupContent.match(
      /<div\s+[^>]*data-sc-content="example-sentence"[^>]*>([\s\S]*?)<\/div>\s*<\/div>/i,
    );
    if (exampleMatch) {
      const exampleContent = exampleMatch[1];

      // Japanese (example-sentence-a)
      const jpMatch = exampleContent.match(
        /<div\s+[^>]*data-sc-content="example-sentence-a"[^>]*>([\s\S]*?)<\/div>/i,
      );
      // English (example-sentence-b)
      const enMatch = exampleContent.match(
        /<div\s+[^>]*data-sc-content="example-sentence-b"[^>]*>([\s\S]*?)<\/div>/i,
      );
...
async function findWordsbyKanji(kanji) {
  try {
    const result = await ankiConnectInvoke("findNotes", 5, {
      query: "deck:Mining",
    });

    const notesInfo = await ankiConnectInvoke("notesInfo", 5, {
      notes: result,
    });
